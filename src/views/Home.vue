        ...
        <div class="hero-layout">
          <aside class="sidebar-cats" aria-label="Categories">
            ...
            <div v-for="cat in categories.slice(0, 12)" :key="cat.id"
              ...
              <span class="si-icon" :style="{ background: cat.color || 'var(--brand-primary, var(--brand-primary, #FF9900))' }">{{ cat.icon || (cat.name || '?')[0] }}</span>
      ...
      <div class="container">
        ...
        <div class="cat-icons-scroll">
          <div v-for="cat in categories" :key="cat.id" class="cat-icon-item" @click="$router.push(`/search?category=${cat.id}`)">
            <div class="ci-circle" :style="{ background: cat.color || 'var(--brand-primary, var(--brand-primary, #FF9900))' }"><span>{{ cat.icon || (cat.name || '?')[0] }}</span></div>
...
const banners = [
  ...
  { tag: '⭐ New Users', title: 'Welcome Bonus $10', desc: 'Sign up and get instant coupon', btn: 'Register', link: '/register', bg: 'linear-gradient(135deg, var(--brand-accent, var(--brand-accent, #007185)), #00a0c4)', emoji: '🎁' },
...
const sideCards = [
  { icon: 'fas fa-user-plus', title: 'New User?', desc: 'Get Coupon', bg: 'linear-gradient(135deg, var(--brand-primary, var(--brand-primary, #FF9900)), #ffad33)' },
  { icon: 'fas fa-percentage', title: 'Daily Deals', desc: 'Up to 50% OFF', bg: 'linear-gradient(135deg, #067D62, #00a07a)' },
  { icon: 'fas fa-truck-fast', title: 'Free Shipping', desc: 'Min. $30 order', bg: 'linear-gradient(135deg, var(--brand-accent, var(--brand-accent, #007185)), #00a0c4)' }
]
...
const getGradient = (name) => { const colors = ['var(--brand-primary, var(--brand-primary, #FF9900))','var(--brand-accent, var(--brand-accent, #007185))','#067D62','var(--brand-dark, var(--brand-dark, #131921))','var(--brand-primary-hover, #E68A00))','var(--brand-nav, #232F3E))']; const idx = (name?.charCodeAt(0)||0)%colors.length; return `linear-gradient(135deg, ${colors[idx]}aa, ${colors[(idx+3)%colors.length]}aa)` }
...
.btn-banner { padding: 0.375rem 1rem; background: var(--brand-primary, var(--brand-primary, #FF9900)); color: var(--brand-dark, var(--brand-dark, #131921)); border-radius: 3px; font-weight: 600; font-size: 0.75rem; text-decoration: none; }
...
.banner-dots span.active { background: var(--brand-primary, var(--brand-primary, #FF9900)); width: 1.25rem; border-radius: 0.25rem; }
...
.flash-icon { font-size: 1rem; color: var(--brand-primary, var(--brand-primary, #FF9900)); }
.flash-title { font-size: 0.9375rem; font-weight: 700; margin: 0; }
.flash-countdown { display: flex; align-items: center; gap: 0.25rem; }
.cd-num { background: var(--brand-dark, var(--brand-dark, #131921)); color: #fff; padding: 0.125rem 0.375rem; border-radius: 2px; font-weight: 700; font-size: 0.75rem; min-width: 1.25rem; text-align: center; }
.cd-sep { font-weight: 700; color: var(--brand-dark, var(--brand-dark, #131921)); }
.flash-see-all { color: var(--brand-accent, var(--brand-accent, #007185)); font-size: 0.8125rem; text-decoration: none; font-weight: 600; }
...
.fc-bar-fill { height: 0.75rem; background: linear-gradient(90deg, var(--brand-primary, var(--brand-primary, #FF9900)), var(--brand-primary-hover, #E68A00)); border-radius: 0.375rem; min-width: 10%; }
...
.ci-name { font-size: 0.6875rem; text-align: center; color: var(--brand-accent, var(--brand-accent, #007185)); white-space: nowrap; }
...
.mall-header h2 i { color: var(--brand-primary, var(--brand-primary, #FF9900)); }
.view-all { color: var(--brand-accent, var(--brand-accent, #007185)); font-size: 0.8125rem; text-decoration: none; font-weight: 600; }
...
.mc-avatar { width: 2.75rem; height: 2.75rem; background: rgba(255,255,255,0.9); border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 1.125rem; font-weight: 700; color: var(--brand-primary, var(--brand-primary, #FF9900)); }
...
.mc-rating { color: var(--brand-primary, var(--brand-primary, #FF9900)); }
.btn-follow-store { padding: 0.25rem 0.875rem; border: 1px solid var(--brand-primary, var(--brand-primary, #FF9900)); color: var(--brand-primary, var(--brand-primary, #FF9900)); background: none; border-radius: 3px; cursor: pointer; font-size: 0.75rem; }
.btn-follow-store:hover { background: var(--brand-primary, var(--brand-primary, #FF9900)); color: var(--brand-dark, var(--brand-dark, #131921)); }
...
.pg-card:hover { border-color: var(--brand-primary, var(--brand-primary, #FF9900)); box-shadow: 0 2px 8px rgba(255,153,0,0.15); transform: translateY(-2px); }
...
.pg-name { font-size: 0.8125rem; color: var(--brand-accent, var(--brand-accent, #007185)); display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; line-height: 1.4; min-height: 2.25rem; margin-bottom: 0.375rem; }
.pg-card:hover .pg-name { color: #c77a00; }
.pg-rating { display: flex; align-items: center; gap: 0.375rem; margin-bottom: 0.25rem; }
.pg-stars { font-size: 0.625rem; color: var(--brand-primary, var(--brand-primary, #FF9900)); }
.pg-reviews { font-size: 0.6875rem; color: var(--brand-accent, var(--brand-accent, #007185)); }
...
.btn-load-more:hover { background: #f7fafa; border-color: var(--brand-accent, var(--brand-accent, #007185)); }
...
.empty-state .btn-primary { margin-top: 1rem; padding: 0.5rem 1.5rem; background: var(--brand-primary, var(--brand-primary, #FF9900)); color: var(--brand-dark, var(--brand-dark, #131921)); border: none; border-radius: 20px; cursor: pointer; font-weight: 600; }
...
.svc-item i { font-size: 1.375rem; color: var(--brand-primary, var(--brand-primary, #FF9900)); flex-shrink: 0; }
