        ...
        <div class="hero-layout">
          <aside class="sidebar-cats" aria-label="Categories">
            ...
            <div v-for="cat in categories.slice(0, 12)" :key="cat.id"
              ...
              <span class="si-icon" :style="{ background: cat.color || 'var(--brand-primary, #FF9900)' }">{{ cat.icon || (cat.name || '?')[0] }}</span>
      ...
      <div class="container">
        ...
        <div class="cat-icons-scroll">
          <div v-for="cat in categories" :key="cat.id" class="cat-icon-item" @click="$router.push(`/search?category=${cat.id}`)">
            <div class="ci-circle" :style="{ background: cat.color || 'var(--brand-primary, #FF9900)' }"><span>{{ cat.icon || (cat.name || '?')[0] }}</span></div>
...
.banner-dots span.active { background: var(--brand-primary, #FF9900); width: 1.25rem; border-radius: 0.25rem; }
...
.cd-sep { font-weight: 700; color: var(--brand-dark, #131921); }
.flash-see-all { color: var(--brand-accent, #007185); font-size: 0.8125rem; text-decoration: none; font-weight: 600; }
...
.fc-bar-fill { height: 0.75rem; background: linear-gradient(90deg, var(--brand-primary, #FF9900), var(--brand-primary-hover, #E68A00)); border-radius: 0.375rem; min-width: 10%; }
...
.ci-name { font-size: 0.6875rem; text-align: center; color: var(--brand-accent, #007185); white-space: nowrap; }
...
.mall-header h2 i { color: var(--brand-primary, #FF9900); }
.view-all { color: var(--brand-accent, #007185); font-size: 0.8125rem; text-decoration: none; font-weight: 600; }
...
.mc-avatar { width: 2.75rem; height: 2.75rem; background: rgba(255,255,255,0.9); border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 1.125rem; font-weight: 700; color: var(--brand-primary, #FF9900); }
...
.filter-tag { padding: 0.375rem 0.875rem; background: #fff; border: 1px solid #D5D9D9; border-radius: 20px; font-size: 0.75rem; cursor: pointer; white-space: nowrap; transition: all 0.15s; min-height: 36px; display: inline-flex; align-items: center; }
...
.pg-name { font-size: 0.8125rem; color: var(--brand-accent, #007185); display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; line-height: 1.4; min-height: 2.25rem; margin-bottom: 0.375rem; }
.pg-card:hover .pg-name { color: #c77a00; }
.pg-rating { display: flex; align-items: center; gap: 0.375rem; margin-bottom: 0.25rem; }
.pg-stars { font-size: 0.625rem; color: var(--brand-primary, #FF9900); }
.pg-reviews { font-size: 0.6875rem; color: var(--brand-accent, #007185); }
...
.btn-load-more:hover { background: #f7fafa; border-color: var(--brand-accent, #007185); }
...
.empty-state .btn-primary { margin-top: 1rem; padding: 0.5rem 1.5rem; background: var(--brand-primary, #FF9900); color: var(--brand-dark, #131921); border: none; border-radius: 20px; cursor: pointer; font-weight: 600; }
...
.svc-item i { font-size: 1.375rem; color: var(--brand-primary, #FF9900); flex-shrink: 0; }
...
@media (max-width: 768px) {
  ...
  .hero-banner { min-height: 9rem; }
  .banner-content { padding: 0.75rem; }
  .banner-text { max-width: 55%; }
  .banner-text h2 { font-size: 0.9375rem; }
  .banner-text p { font-size: 0.6875rem; margin-bottom: 0.5rem; }
  .banner-visual { font-size: 1.75rem; }
  .btn-banner { padding: 0.375rem 0.75rem; font-size: 0.6875rem; }
  .product-grid-amazon { grid-template-columns: repeat(3, 1fr); gap: 0.375rem; }
  .mall-grid { grid-template-columns: repeat(2, 1fr); }
  .services-row { grid-template-columns: repeat(2, 1fr); gap: 0.75rem; }
  .svc-item { gap: 0.5rem; }
  .svc-item i { font-size: 1.125rem; }
...
@media (max-width: 480px) {
  ...
  .pg-body { padding: 0.375rem; }
  .pg-name { font-size: 0.75rem; min-height: 2rem; }
  .pg-price { font-size: 0.875rem; }
  .hero-side-cards { grid-template-columns: 1fr; }
  .side-card { padding: 0.5rem; display: flex; align-items: center; gap: 0.5rem; }
  .side-card small { display: none; }
  .banner-content { padding: 0.625rem; }
  .banner-text { max-width: 65%; }
  .banner-text h2 { font-size: 0.8125rem; line-height: 1.3; }
  .banner-text p { font-size: 0.625rem; display: none; }
  .banner-visual { font-size: 1.5rem; }
  .flash-card { min-width: 6rem; }
  .fc-info { padding: 0.25rem; }
  .fc-price { font-size: 0.75rem; }
  .mall-grid { grid-template-columns: repeat(2, 1fr); gap: 0.375rem; }
  .cat-icon-item { min-width: 3.75rem; }
  .seo-footer-grid { grid-template-columns: 1fr; gap: 1.25rem; }
  .btn-follow-store { padding: 0.375rem 0.75rem; font-size: 0.6875rem; min-height: 32px; }
