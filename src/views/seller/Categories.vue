<template>
  ...
  <div class="seller-categories">
    ...
    <div v-if="selectedCat" class="modal-overlay" @click.self="selectedCat = null">
      <div class="modal">
        <div class="modal-header" :style="{ background: selectedCat.color || 'var(--brand-primary, #FF9900)' }">
...
.filters input:focus { outline: none; border-color: var(--brand-primary, #FF9900); }
...
.product-count { font-size: 11px; color: var(--brand-primary, #FF9900); font-weight: 600; background: #ffe0e6; padding: 3px 10px; border-radius: 12px; }
...
.stat strong { display: block; font-size: 22px; color: var(--brand-primary, #FF9900); margin-bottom: 4px; }
.stat span { font-size: 12px; color: #999; }
.btn-browse { width: 100%; padding: 12px; background: var(--brand-primary, #FF9900); color: #fff; border: none; border-radius: 8px; cursor: pointer; font-size: 14px; font-weight: 600; }
