  ...
  <div class="seller-categories">
    ...
    <div v-else class="category-grid">
      <div v-for="cat in filtered" :key="cat.id" class="category-card" @click="selectedCat = cat">
        <div class="cat-icon" :style="{ background: cat.color || '#fe2c55' }">{{ cat.icon || cat.name[0] }}</div>
        <h3>{{ cat.name }}</h3>
    ...
    <div v-if="selectedCat" class="modal-overlay" @click.self="selectedCat = null">
      <div class="modal">
        <div class="modal-header" :style="{ background: selectedCat.color || '#fe2c55' }">
...
.filters input:focus { outline: none; border-color: #fe2c55; }
...
.product-count { font-size: 11px; color: #fe2c55; font-weight: 600; background: #ffe0e6; padding: 3px 10px; border-radius: 12px; }
...
.stat strong { display: block; font-size: 22px; color: #fe2c55; margin-bottom: 4px; }
.stat span { font-size: 12px; color: #999; }
.btn-browse { width: 100%; padding: 12px; background: #fe2c55; color: #fff; border: none; border-radius: 8px; cursor: pointer; font-size: 14px; font-weight: 600; }
</style>
