<template>
  <div class="admin-report">
    ...
    <div class="stats-grid">
      <div class="stat-card"><div class="stat-icon" style="background:#fe2c55"><i class="fas fa-box"></i></div><div><h3>{{ products.length }}</h3><p>Total Products</p></div></div>
    ...
    <div class="grid-2">
      ...
      <div class="card"><h2>Products by Category</h2><div class="category-bars"><div v-for="c in categoryStats" :key="c.name" class="cat-bar"><div class="cat-info"><span>{{ c.name }}</span><span>{{ c.count }}</span></div><div class="cat-fill"><div class="cat-progress" :style="{ width: c.pct + '%', background: c.color || '#fe2c55' }"></div></div></div></div></div>
    </div>
...
const categoryStats = computed(() => {
  ...
  return Object.entries(map).map(([name, count]) => ({ name, count, pct: (count/max)*100, color: '#fe2c55' })).sort((a,b) => b.count-a.count)
})
