...
const editProduct = (product) => window.__toast?.show('Edit: ' + product.name)

const deleteProduct = async (id) => {
  if (!confirm('Delete this product?')) return
  try { await supabase.from('products').delete().eq('id', id) } catch(_e) { console.error('Products.vue:', _e); window.__toast?.show('Operation failed', 'error') }
  await loadProducts()
}
