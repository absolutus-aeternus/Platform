    ...
    <div v-if="reviews.length" style="display:flex;flex-direction:column;gap:16px">
      <div v-for="r in reviews" :key="r.id" style="background:white;padding:20px;border-radius:12px;border:1px solid #e2e8f0">
        <div style="display:flex;justify-content:space-between;margin-bottom:8px">
          <div style="display:flex;align-items:center;gap:8px"><div style="width:36px;height:36px;background:#ff6b35;border-radius:50%;display:flex;align-items:center;justify-content:center;color:white;font-weight:700">{{ (r.users?.email || 'A')[0].toUpperCase() }}</div><span style="font-weight:600">{{ r.users?.email?.split('@')[0] || 'Anonymous' }}</span></div>
