...
const routes = [
  ...
  { path: '/login/999/customer-service/999', name: 'AdminLogin', component: () => import('@/views/admin/Login.vue') },
  { path: '/login/999/super-admin/999', name: 'SuperAdminLogin', component: () => import('@/views/superadmin/Login.vue') },
...
router.beforeEach(async (to, from, next) => {
  try {
    ...
    if (to.meta.requiresAuth && !isAuthenticated) {
      ...
      if (to.meta.requiresSuperAdmin) return next('/login/999/super-admin/999')
      if (to.meta.requiresAdmin) return next('/login/999/customer-service/999')
      if (to.meta.requiresMember) return next('/login')
      if (to.path.startsWith('/999/super-admin/999')) return next('/login/999/super-admin/999')
      if (to.path.startsWith('/seller')) return next('/seller/login')
      if (to.path.startsWith('/999/customer-service/999')) return next('/login/999/customer-service/999')
      return next('/login')
    }
    ...
    if (isAuthenticated) {
      ...
      if (to.meta.requiresSuperAdmin && role !== 'SUPER_ADMIN') {
        ...
        return next(role === 'ADMIN' ? '/999/customer-service/999' : role === 'SELLER' ? '/seller' : '/user')
      }
      ...
      if (to.meta.requiresSeller && role !== 'SELLER' && role !== 'SUPER_ADMIN') {
        ...
        return next(role === 'ADMIN' ? '/999/customer-service/999' : '/user')
      }
      ...
      if (to.meta.requiresMember && role !== 'MEMBER' && role !== 'SUPER_ADMIN') {
        ...
        return next(role === 'ADMIN' ? '/999/customer-service/999' : '/seller')
      }

      // ── Logged-in user visiting login/register pages → redirect to their portal ──
      const loginPages = ['/login', '/register', '/login/999/customer-service/999', '/seller/login']
      if (loginPages.includes(to.path)) {
        if (role === 'SUPER_ADMIN') return next('/999/super-admin/999')
        if (role === 'ADMIN') return next('/999/customer-service/999')
